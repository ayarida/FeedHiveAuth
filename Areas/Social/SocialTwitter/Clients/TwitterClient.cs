...
namespace FeedHiveAuth.Areas.Social.SocialTwitter.Clients
{
    public abstract class TwitterClient : RestClientService
    {
        ...
        public TwitterClient(TwitterConfigs configs, string node, string url = "https://api.twitter.com/1.1", TwitterCredentials token = null) : base(url + (string.IsNullOrWhiteSpace(node) || node.StartsWith("/") ? "" : "/") + node)
        {
